  ...
  <b-container>
    <SpinnerComponent v-if="loading"></SpinnerComponent>
      <b-row class="mb-4 justify-content-between" v-else>
        <p class="col-12 h3 mb-5">Hai totalizzato {{ points }} punt{{ points === 1 ? 'o' : 'i' }}!</p>
...
export default {
  name: 'PostGameStatsComponent',
  components: {
    SpinnerComponent,
  },
  props: ['gameType', 'points'],
  data() {
    return {
      loading: true,
      records: [],
  ...
  methods: {
    updateLeaderboard() {
      return fetch(apiURL + `leaderboards/${this.gameType}`, {
        method: 'GET',
    ...
    updatePlayerScore(username) {
      return fetch(apiURL + `users/${username}/games/${this.gameType}`, {
        method: 'POST',
        credentials: "same-origin",
        headers: {
    ...
    getUserPoints(username) {
      return fetch(apiURL + `users/${username}/games/${this.gameType}`, {
        ...
        then(data => {
          return { last: data.last ?? 0, best: data.best ?? 0 };
        }).catch(err => {
          console.error(err);
          return { last: 0, best: 0 };
        });
    ...
    updateScores() {
      fetch(apiURL + "auth", {
        ...
        if (response.ok) {
          ...
          this.lastScore = (await this.getUserPoints(data.username)).last;
          await this.updatePlayerScore(data.username);
          this.bestScore = (await this.getUserPoints(data.username)).best;
        } else {
          this.logged = false;
        }
      }).catch(err => {
